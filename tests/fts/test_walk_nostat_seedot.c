...
int main(void) {
    ...
    fts_run_walk("PHYSICAL NOSTAT SEEDOT", tree.abs_root, roots, FTS_PHYSICAL | FTS_NOSTAT | FTS_SEEDOT, false, false,
                 &s);
    free(roots);
    fts_check_soft(s.n_dirs >= 4, "nostat traversal reached nested directories");
