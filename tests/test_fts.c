...
static int build_tree(const char* root) {
    ...
    if (ensure_dir(path, 0755) == -1) return -1;
    ...
    if (!unread) return -1;
    if (ensure_dir(unread, 0700) == -1) { free(unread); return -1; }
    if (chmod(unread, 0000) == -1) { free(unread); return -1; }
    free(unread);
    ...
    if (!link_to_a || symlink("../a", link_to_a) == -1) { free(link_to_a); return -1; }
    free(link_to_a);

    char* link_to_nowhere = join2(path, "link_to_nowhere");
    if (!link_to_nowhere || symlink("./nope", link_to_nowhere) == -1) { free(link_to_nowhere); return -1; }
    free(link_to_nowhere);

    char* cyc = join2(path, "cycle");
    if (!cyc || symlink(".", cyc) == -1) { free(cyc); return -1; }
    free(cyc);
...
static void print_ent(const char* root_prefix, const FTSENT* e) {
    ...
    if (lst_ok == 0 && S_ISLNK(st.st_mode)) {
        if (readlink_buf(full, lnk, sizeof(lnk)) < 0) snprintf(lnk, sizeof(lnk), "<readlink err:%s>", strerror(errno));
    }

    printf("level=%-3d info=%-9s name=\"%s\" path=\"%s\" acc=\"%s\" full=\"%s\"",
           e->fts_level, info_label(e->fts_info), e->fts_name, e->fts_path, e->fts_accpath ? e->fts_accpath : "", full);
    if (lnk[0]) printf(" -> \"%s\"", lnk);
    printf("\n");
}
...
static void reset_stats(WalkStats* s) { memset(s, 0, sizeof(*s)); }
static void account(WalkStats* s, const FTSENT* e) {
    switch (e->fts_info) {
        case FTS_D: s->n_dirs++; break;
        case FTS_DP: s->n_post++; break;
        case FTS_F: s->n_files++; break;
        case FTS_SL: s->n_symlinks++; break;
        case FTS_SLNONE: s->n_dangling++; break;
        case FTS_DNR:
        case FTS_ERR:
        case FTS_NS: s->n_errors++; break;
        default: break;
    }
}

/* run one traversal with options and diagnostics */
static void run_walk(const char* label, const char* root_prefix, char* const* roots, int opts, bool use_cmp, bool try_instr, WalkStats* out) {
    printf(C_BLU "\n== %s ==\n" C_RST, label);
    printf("options:");
    if (opts & FTS_NOCHDIR)  printf(" FTS_NOCHDIR");
    if (opts & FTS_PHYSICAL) printf(" FTS_PHYSICAL");
    if (opts & FTS_LOGICAL)  printf(" FTS_LOGICAL");
    if (opts & FTS_COMFOLLOW)printf(" FTS_COMFOLLOW");
    if (opts & FTS_SEEDOT)   printf(" FTS_SEEDOT");
    if (opts & FTS_NOSTAT)   printf(" FTS_NOSTAT");
    if (opts & FTS_XDEV)     printf(" FTS_XDEV");
    printf("\n");
...
static int build_symlink_loop(const char* abs_root) {
    ...
    if (ensure_dir(a, 0755) == -1) return -1;
    if (ensure_dir(b, 0755) == -1) return -1;

    char* a_to_b = join2(a, "toB");
    char* b_to_a = join2(b, "toA");
    if (!a_to_b || !b_to_a) { free(a_to_b); free(b_to_a); return -1; }

    /* loopA/toB -> ../loopB and loopB/toA -> ../loopA */
    if (symlink("../loopB", a_to_b) == -1) { free(a_to_b); free(b_to_a); return -1; }
    if (symlink("../loopA", b_to_a) == -1) { free(a_to_b); free(b_to_a); return -1; }
...
static void test_many_children_sorted(const char* abs_root, int n) {
    ...
    while ((e = fts_read(f)) != NULL) {
        if (e->fts_level == 1) {
            if (seen == 0) first = strdup(e->fts_name);
            else if (seen == 1) second = strdup(e->fts_name);
            seen++;
            if (seen >= 2) break;
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        ...
        printf("\nsummary: dirs=%zu files=%zu symlinks=%zu dangling=%zu post=%zu errors=%zu\n",
               s.n_dirs, s.n_files, s.n_symlinks, s.n_dangling, s.n_post, s.n_errors);
