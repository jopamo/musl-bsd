...
static const struct uparam_name uparam_names[] = {{"dup-args", true, offsetof(struct uparams, dup_args)},
...
%.*s: Unknown ARGP_HELP_FMT parameter"),
                ...
                __argp_failure(
                    state, 0, 0,
                    dgettext(state == NULL ? NULL : state->root_argp->argp_domain, "Garbage in ARGP_HELP_FMT: %s"),
                    var);
                break;
            }
        }
...
static struct hol* make_hol(const struct argp* argp, struct hol_cluster* cluster) {
    ...
    if (opts) {
        ...
        for (o = opts, entry = hol->entries; !oend(o); entry++) {
            ...
            entry->group = cur_group = o->group ? o->group : ((!o->name && !o->key) ? cur_group + 1 : cur_group);
...
static int hol_entry_short_iterate(
    const struct hol_entry* entry,
    int (*func)(const struct argp_option* opt, const struct argp_option* real, const char* domain, void* cookie),
    const char* domain,
    void* cookie) {
    unsigned nopts;
    int val = 0;
...
static inline int hol_entry_long_iterate(
    const struct hol_entry* entry,
    int (*func)(const struct argp_option* opt, const struct argp_option* real, const char* domain, void* cookie),
    const char* domain,
    void* cookie) {
...
static inline int until_short(const struct argp_option* opt,
...
static void comma(unsigned col, struct pentry_state* pest) {
    if (pest->first) {
        ...
        if (cl && cl->header && *cl->header && (!pe || (pe->cluster != cl && !hol_cluster_is_child(pe->cluster, cl))))
...
static void hol_entry_help(struct hol_entry* entry,
    ...
    if (pest.first) {
        ...
        const char* tstr =
            real->doc ? dgettext_safe(state == NULL ? NULL : state->root_argp->argp_domain, real->doc) : 0;
...
optional for any corresponding short options.");
        const char* fstr = filter_doc(tstr, ARGP_KEY_HELP_DUP_ARGS_NOTE, state ? state->root_argp : 0, state);
        if (fstr && *fstr) {
            __argp_fmtstream_putc(stream, '\n');
            __argp_fmtstream_puts(stream, fstr);
            __argp_fmtstream_putc(stream, '\n');
...
static void hol_usage(struct hol* hol, argp_fmtstream_t stream) {
    if (hol->num_entries > 0) {
        ...
        for (entry = hol->entries, nentries = hol->num_entries; nentries > 0; entry++, nentries--)
            hol_entry_short_iterate(entry, add_argless_short_opt, entry->argp->argp_domain, &snao_end);
        ...
        for (entry = hol->entries, nentries = hol->num_entries; nentries > 0; entry++, nentries--)
            hol_entry_short_iterate(entry, usage_argful_short_opt, entry->argp->argp_domain, stream);
...
static int argp_doc(const struct argp* argp,
    ...
    /* We have to filter the doc strings.  */
    {
        ...
        text = (*argp->help_filter)(post ? ARGP_KEY_HELP_POST_DOC : ARGP_KEY_HELP_PRE_DOC, inp_text, input);
    }
    else
