...
static const struct argp_option argp_default_options[] = {
    ...
    {"HANG", OPT_HANG, "SECS", OPTION_ARG_OPTIONAL | OPTION_HIDDEN, N_("Hang for SECS seconds (default 3600)"), 0},
    {0, 0, 0, 0, 0, 0}};
    ...
    switch (key) {
        case 'V':
            if (argp_program_version_hook)
                ...
                __argp_error(state, dgettext(state->root_argp->argp_domain, "(PROGRAM ERROR) No version known!?"));
...
static const struct argp_option* find_short_option(struct parser* parser, int key, struct group** p) {
    struct group* group;
...
static enum match_result match_option(const char* arg, const char* name) {
    unsigned i, j;
    for (i = j = 0;; i++, j++) {
        switch (arg[i]) {
            case '\0':
...
static void parser_convert(struct parser* parser, const struct argp* argp) {
    struct parser_convert_state cvt;
...
static error_t parser_finalize(struct parser* parser, error_t err, int arg_ebadkey, int* end_index) {
    ...
    if (!err) {
           ...
           just a few more times... */
        {
            ...
            for (group = parser->egroup - 1; group >= parser->groups && (!err || err == EBADKEY); group--)
        ...
        /* No way to return the remaining arguments, they must be bogus. */
        {
            if (!(parser->state.flags & ARGP_NO_ERRS) && parser->state.err_stream)
                fprintf(parser->state.err_stream, dgettext(parser->argp->argp_domain, "%s: Too many arguments\n"),
                        parser->state.name);
            err = EBADKEY;
        }
    }
...
static void exchange(struct parser* parser) {
...
static enum arg_type classify_arg(struct parser* parser, char* arg, char** opt) {
    if (arg[0] == '-')
...
int __THROW __option_is_short(__const struct argp_option* __opt) {
...
int __THROW __option_is_end(__const struct argp_option* __opt) {
