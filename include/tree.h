...
#define RB_FOREACH_SAFE(x, name, head, y) \
    for ((x) = RB_MIN(name, head); ((x) != NULL) && ((y) = name##_RB_NEXT(x), 1); (x) = (y))
...
#define RBT_FOREACH_SAFE(_e, _name, _head, _n) \
    for ((_e) = RBT_MIN(_name, (_head)); (_e) != NULL && ((_n) = RBT_NEXT(_name, (_e)), 1); (_e) = (_n))

#define RBT_FOREACH_REVERSE(_e, _name, _head) \
    for ((_e) = RBT_MAX(_name, (_head)); (_e) != NULL; (_e) = RBT_PREV(_name, (_e)))
